...
// include or declare snoise here if it's not globally available
// ...
...
void main() {
  ...
  float n = snoise(vec3(p.x * 0.6 + adjustedTime * 0.2, p.y * 0.4 + adjustedTime * 0.3, p.z * 0.2 + adjustedTime * 0.2));
  // float n = snoise(vec3(vUv * 0.6, time * 0.5)) * 0.5 + 0.5;
  p += n * 0.6;
